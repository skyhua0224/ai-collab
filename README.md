...
3. **双执行后端**：`direct` 适合当前终端顺序执行，`tmux` 适合可视化协作、日志记录与会话回放
...
说明：默认 `direct` 语义是“单终端执行后端”。

- 单 Agent 计划：直接由当前 Agent 在当前终端执行
- 多 Agent 计划：仍在当前终端执行，但按 workflow 阶段顺序调用不同 Agent，不创建 `tmux` pane
...
说明：`tmux` 只用于“可执行的多 Agent 可视化编排”。
如果计划编辑后变成单 Agent，或所有步骤都落到同一个 Agent，执行目标页会禁用 `tmux`；这时仍可继续使用“直接执行”在当前终端运行。
...
- 🐧 QQ: 2110591491
- 🔗 GitHub Issues: [提交 Issue](https://github.com/skyhua0224/ai-collab/issues)
